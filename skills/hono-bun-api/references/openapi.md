...
`createApiRouter()` retorna un `OpenAPIHono<AppEnv>` pre-configurado con un
**defaultHook** que uniformizia las validaciones Zod fallidas a **HTTP 422** con
shape `AppError.Validation`. Sin esto, `@hono/zod-openapi` responde 400 con su
propio body — incoherente con el resto del Result pattern.

Implementación en `shared/hono/router.ts`:
...
**Importante:** cada feature `routes.ts` debe usar `createApiRouter()` para sus
