...
import { DEFAULT_EXAMPLES_LIMIT, EXAMPLE_STATUSES, MAX_EXAMPLES_LIMIT } from './examples.constants';

export const ExampleStatus = z.enum(EXAMPLE_STATUSES);
...
export const ListExamplesQuery = z
  .object({
    ...
    limit: z.coerce.number().int().min(1).max(MAX_EXAMPLES_LIMIT).default(DEFAULT_EXAMPLES_LIMIT),
  })
